---
title: "Omixer: Multivariate and reproducible randomization of omics samples 
    with intuitive visualization of resulting layouts"
author: 
- Lucy Sinke
- Davy Cats
- Bas Heijmans*
...
intuitive visualizations of sample layouts for the wet lab.

## Installation ##

The **Omixer** package can be installed using 
[**devtools**](https://github.com/hadley/devtools) in R.
...
```{r example_plate_col, fig.wide=TRUE, out.width='100%', echo=FALSE}
knitr::include_graphics(file.path(here(), "vignettes", "omixer_48col.png"))
```

If you instead specify `div="row"`, the rows will be treated as batches. 

```{r example_plate_row, fig.wide=TRUE, out.width='100%', echo=FALSE}
knitr::include_graphics(file.path(here(), "vignettes", "omixer_48row.png"))
```
...
```{r example_plate_block, fig.wide=TRUE, out.width='100%', echo=FALSE}
knitr::include_graphics(file.path(here(), "vignettes", 
    "omixer_48colblock.png"))
```
...
We create toy data, representing 96 samples to be sent off for RNA sequencing. 
We have information about the age, sex, and smoking status of the individuals 
alongside sample isolation dates, and want to optimize distribution of these 
across lanes on the flowcell to minimize batch effects.

```{r rna_toy_data}
set.seed(999)

sampleList <- tibble(sampleId=str_pad(1:96, 3, pad="0"),
    sex=as_factor(sample(c("male", "female"), 96, replace=TRUE)), 
    age=round(rnorm(96, mean=30, sd=8), 0), 
    smoke=as_factor(sample(c("current", "ex", "never"), 96, 
        replace=TRUE)),
...
omixerLayout <- omixerRand(sampleList, sampleId="sampleId", 
    block="block", iterNum=10, wells=96, div="row", 
...
methylation profiling. These samples will be used to investigate how DNA 
methylation changes over time in 4 tissues from 77 individuals. 

Given our primary research question, we would like to keep the two timepoints 
adjacent on the array but randomize their order. Additionally, we want to 
reserve the final chip (8 wells) on each plate for control samples. This 
amounts to sending 7 96-well plates for profiling.

## Creating Toy Data ##

As well as a sample ID, we need to tell **Omixer** which variables specify 
paired sample blocks. Randomization variables in our example are tissue, sex, 
age, body mass index (BMI), and isolation date. 
...
biological factors across chips and plates. 
...
* Age and chip (τ = -0.064, p = 0.026)
* Isolation date and plate (τ = -0.073, p = 0.012)
* Tissue and plate (τ = 0.072, p = 0.026)

There is also borderline evidence for a correlation between:

* BMI and chip (τ = 0.049, p = 0.083)
* BMI and plate (τ = 0.052, p = 0.075)
...
of the optimal layout is printed, and this can be input into the following 
function for quick and easy regeneration.

The layout above was generated with a seed of 43956.

```{r omixer_specific, eval=FALSE}
omixerLayout <- omixerSpecific(sampleList, sampleId="sampleId", 
    block="block", seed=43956, wells=96, div="col", plateNum=7, 
    randVars=randVars, mask=mask)
```
...
accurately to minimize sample mixups. The `omixerSheet` function smooths this 
transition by creating PDFs of sample layouts that are intuitive for manual 
pipetting. 
...
```{r example_sheet, fig.wide = TRUE, out.width = '100%', echo=FALSE}
knitr::include_graphics(file.path(here(), "vignettes", 
    "omixer_sample_sheets.png"))
```
